...
function fnct_RecordTime() {
	...
	var data = fnct_CallPostAjax("/manage/getMaxWorkday.ajax", params);			
...
function fnct_CheckPass() {
	if($("#renewPass").val() == "" || $("#newPass").val() == "") {
		swal("안내", "비밀번호를 입력해주세요.", "error");	
		return;
	}
	
	if($("#renewPass").val() != $("#newPass").val()) {
		swal("안내", "비밀번호가 다릅니다.", "error");	
		return;
	}
}
...
function fnct_EditPass() {	
	...
	var data = fnct_CallPostAjax("/login/checkPass.ajax", params);		
	if(data.status == "success") {
		swal("안내", "비밀번호가 변경되었습니다.", "success");		
	} else {
		swal("안내", "비밀번호가 틀렸습니다.", "error");
	}
...
function fnct_RecordCommute(flag) {
	...
	var data = fnct_CallPostAjax("/manage/recordCommute.ajax", params);		
	if(data.status == "success") {
		swal("안내", "확인되었습니다.", "success");
	}
}
