...
<script>
function fnct_WriteMailfromSave() {
	var params = {
			"mailStatus" : "S",
			"mailNo"     : $("#mailNo").val()
	};
	Object.defineProperty(params, "${_csrf.parameterName}", {value : "${_csrf.token}", writable : true, enumerable : true});	
	$("#mailSpace").load("/mail/writeMail.do", params);
}

$(document).ready(function() {
	mailStatus = "U";		// 메일 페이지 구분자 (S: 받은편지 / M:내게쓴편지 / U:임시보관함 / P:휴지통)
		...
		<div class="card-header">
	  	...
	  	<div class="card-footer">
	  		<div class="text-center">
		  		<button type="button" class="btn btn-outline-secondary" id="btn-reply" onclick="fnct_WriteMailfromSave()">작 성</button>			
			</div>			
	  	</div>
	</div>
</div>